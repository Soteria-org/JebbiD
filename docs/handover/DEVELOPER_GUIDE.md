...
2. `app/auth/reset-password/route.js` — receives the emailed link (`?token_hash=...&type=recovery`), verifies it, establishes a session, then redirects to `/portal?resetPassword=1`. Deliberately a separate route from `app/auth/confirm/route.js` (not reused) — confirming a signup and recovering a password are different trust events, and this one must *not* call `createInvestorProfileRows`.
3. **No separate page for setting the new password.** `app/portal/page.js` reads `?resetPassword=1` and passes it to `JBDocsApp` as `resetPasswordRequested`, which tells `LoginScreen` to open directly in its `"reset"` mode (`ResetPasswordForm` inside `src/features/auth/LoginScreen.jsx`) instead of the normal sign-in form. Clicking the emailed link is the whole flow — no code to type, no extra page. Submitting signs the recovery session out (see `completePasswordReset()`'s comment — the SPA shell has no existing-session bootstrap, so leaving it signed in would just be a dangling unused cookie) and drops the user back into the normal sign-in form with a "Password updated" toast.
