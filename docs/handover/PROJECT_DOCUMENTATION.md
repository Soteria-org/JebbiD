...
3. **Resend webhook diagnosed, not yet resolved.** `email_events` has zero rows despite correctly-configured Resend webhook config and confirmed-delivered emails. Root cause narrowed to `RESEND_WEBHOOK_SECRET` — either not set in the Vercel **Production** environment, or set without a redeploy since. **This needs a human with Vercel dashboard access** — no Vercel project was reachable from the tooling available during this session. See §6.
4. **Freeze/unfreeze — the pause threat now has teeth.** Risk & Compliance Monitor's KYC/missing-info/dormant findings already warned investors their account "may be paused," but nothing enforced it. Now:
...
- **Resend webhook not receiving events.** Config on Resend's side is correct (endpoint, events, enabled) and confirmation emails are genuinely being delivered — but `email_events` has zero rows. Almost certainly `RESEND_WEBHOOK_SECRET` missing from Vercel's **Production** environment, or set without a redeploy since. The exact signing secret Resend has on file: check it matches what's in Vercel → Project → Settings → Environment Variables → `RESEND_WEBHOOK_SECRET`, then redeploy. No Vercel project was reachable from the tooling available in this session to verify/fix directly — needs a human with dashboard access.
