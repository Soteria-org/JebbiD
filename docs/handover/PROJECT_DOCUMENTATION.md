...
5. **Forgot/reset password — didn't exist before.** "Forgot your password?" on the sign-in screen → `requestPasswordReset()` sends a real recovery email via Supabase Auth (always reports success regardless of whether the account exists, to prevent using the form to enumerate registered accounts) → `/auth/reset-password` verifies the emailed link and establishes a session (kept as a separate route from `/auth/confirm` since they're different trust events) → a standalone "set new password" page enforces the same password policy used everywhere else, then sends the user back to sign in. **Requires a manual step:** `/auth/reset-password` must be added to Supabase Dashboard → Authentication → URL Configuration → Redirect URLs, same as `/auth/confirm` already needed.
