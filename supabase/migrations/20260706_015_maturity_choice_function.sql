-- DECISION (was flagged, not silently picked): reinvestment/maturity choices create
-- a new investment_positions row WITHOUT a deposit_submission behind it. Modeling
-- this as a fake auto-approved deposit was rejected — a deposit_submissions row
-- means "money physically arrived by mobile money/bank," which isn't true here, and
-- polluting that table with synthetic rows would corrupt its audit meaning. This
-- dedicated function is the alternative: it creates positions directly, with its own
-- explicit audit trail, keeping deposit_submissions meaning exactly what it says.
--
-- Single Postgres function (not multiple app-side calls) because this must be
-- atomic: close old position + open new position + maybe create a withdrawal, all
-- or nothing. `for update` locks the row so a double-click can't process the same
-- maturity decision twice.
...
begin
  ...
  if p_choice in ('reinvest','switch_package') then
    ...
    -- Placeholder payment_method/payout_details — known gap: this flow doesn't yet
    -- collect real payout details (mobile money number, bank account) the way
    -- RequestWithdrawalModal does for a manual withdrawal. Staff will need to follow
    -- up with the investor before actually paying this out. Documented, not hidden.
