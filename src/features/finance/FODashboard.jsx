...
import { daysBetween, fmtDate, fmtUGX } from "@/lib/format";
import { C } from "@/lib/theme";

export function FODashboard({ ctx }) {
  const m = useStaffMetrics(ctx);
  const weeklyApproved = ctx.auditLog.filter((a) => a.action === "Deposit Approved" && daysBetween(a.timestamp, TODAY) <= 7).length;
  const sortedDeposits = [...m.pendingDeposits].sort((a, b) => a.createdAt - b.createdAt);
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1.2fr 1fr", gap: 18 }}>
        <Card>
          ...
          {sortedDeposits.length === 0 ? <div style={{ fontSize: 13, color: C.inkFaint, padding: "20px 0" }}>No deposits awaiting review.</div> :
            sortedDeposits.slice(0, 5).map((p) => {
              const investor = ctx.getInvestor(p.investorId);
              return (
                <div key={p.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "11px 0", borderBottom: "1px solid " + C.line }}>
                  <div><div style={{ fontWeight: 600, fontSize: 13.5, color: C.ink }}>{investor.fullName}</div><div style={{ fontSize: 12, color: C.inkFaint }}>{p.id} · {fmtUGX(p.amount)} · waiting {daysBetween(p.createdAt, TODAY)}d</div></div>
        ...
        <Card>
          ...
          {m.upcomingMaturities.filter((p) => daysBetween(TODAY, p.maturityDate) <= 30).length === 0 ? <div style={{ fontSize: 13, color: C.inkFaint }}>None upcoming.</div> :
            m.upcomingMaturities.filter((p) => daysBetween(TODAY, p.maturityDate) <= 30).map((p) => {
              const investor = ctx.getInvestor(p.investorId);
              return (
                <div key={p.id} style={{ padding: "10px 0", borderBottom: "1px solid " + C.line }}>
                  ...
                  <div style={{ fontSize: 12, color: C.inkFaint }}>{p.id} · matures {fmtDate(p.maturityDate)} · {investor.phone}</div>
                </div>
              );
            })}
        </Card>
