...
export function LoginScreen({ ctx, initialMode }) {
  ...
  return (
    <div className="jbd-login-shell" style={{ minHeight: "100vh", display: "flex", background: C.pageBg, fontFamily: FONT_BODY }}>
      <style>{`
        @media (max-width: 860px) {
          .jbd-login-shell { flex-direction: column; }
          .jbd-login-promo { min-width: 0 !important; padding: 28px 24px !important; }
          .jbd-login-desc, .jbd-login-stats, .jbd-login-rule { display: none !important; }
          .jbd-login-headline { font-size: 22px !important; margin-bottom: 0 !important; max-width: none !important; }
          .jbd-login-form-wrap { min-width: 0 !important; padding: 28px 24px !important; }
        }
        @keyframes jbd-ring-drift-a { 0%, 100% { transform: translate(0, 0) scale(1); } 50% { transform: translate(-10px, 12px) scale(1.04); } }
        @keyframes jbd-ring-drift-b { 0%, 100% { transform: translate(0, 0) scale(1); } 50% { transform: translate(8px, -8px) scale(0.97); } }
        @keyframes jbd-sheen-sweep { 0% { transform: translateX(-60%) rotate(8deg); } 100% { transform: translateX(60%) rotate(8deg); } }
        @keyframes jbd-fade-up { from { opacity: 0; transform: translateY(10px); } to { opacity: 1; transform: translateY(0); } }
        .jbd-login-ring-a { animation: jbd-ring-drift-a 17s ease-in-out infinite; }
        .jbd-login-ring-b { animation: jbd-ring-drift-b 13s ease-in-out infinite; }
        .jbd-login-sheen { animation: jbd-sheen-sweep 9s ease-in-out infinite alternate; }
        .jbd-login-fade-1 { animation: jbd-fade-up 0.6s ease both; }
        .jbd-login-fade-2 { animation: jbd-fade-up 0.6s ease 0.1s both; }
        .jbd-login-fade-3 { animation: jbd-fade-up 0.6s ease 0.2s both; }
      `}</style>
      <div className="jbd-login-promo" style={{ flex: 1, background: "radial-gradient(circle at 18% 12%, " + C.brand + ", " + C.brandDark + " 62%)",
        ...
        <div className="jbd-login-ring-a" style={{ position: "absolute", right: -60, top: -60, width: 280, height: 280, border: "1.5px solid rgba(255,255,255,0.22)", borderRadius: "50%" }} />
        <div className="jbd-login-ring-b" style={{ position: "absolute", right: -10, top: 30, width: 180, height: 180, border: "1.5px solid rgba(255,255,255,0.16)", borderRadius: "50%" }} />
        <div className="jbd-login-sheen" style={{ position: "absolute", top: "-20%", left: "30%", width: "50%", height: "140%", background: "linear-gradient(90deg, rgba(255,255,255,0) 0%, rgba(255,255,255,0.07) 50%, rgba(255,255,255,0) 100%)", pointerEvents: "none" }} />
        <div style={{ position: "relative" }}>
          <div className="jbd-login-fade-1" style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 44 }}>
            <Logo size={38} />
            <div style={{ fontFamily: FONT_MONO, fontSize: 13, letterSpacing: 2, color: C.sidebarText }}>JEBBIDOX</div>
          </div>
          <div className="jbd-login-headline jbd-login-fade-2" style={{ fontFamily: FONT_DISPLAY, fontSize: 36, fontWeight: 500, lineHeight: 1.22, marginBottom: 18, color: C.white, maxWidth: 440 }}>
            Small, consistent investments build great wealth.
          </div>
          <div className="jbd-login-rule" style={{ height: 1, width: 64, background: "rgba(255,255,255,0.5)", margin: "24px 0" }} />
          <div className="jbd-login-desc jbd-login-fade-3" style={{ fontSize: 14, color: C.sidebarText, lineHeight: 1.65, maxWidth: 400 }}>
        ...
        <div className="jbd-login-stats jbd-login-fade-3" style={{ position: "relative", display: "flex", gap: 28, flexWrap: "wrap" }}>
      ...
      <div className="jbd-login-form-wrap" style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", padding: 32, minWidth: 320 }}>
